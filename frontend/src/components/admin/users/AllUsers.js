...
const AllUser = (props) => {
  ...
  const editUser = (cId, type, des, status) => {
    if (type) {
      dispatch({
                ...
                return (
                  <UserTable
                    ...
                    editCat={(cId, type, des, status) =>
                      editUser(cId, type, des, status)
                    }
                    deleteCat={(cId) => deleteUserReq(cId)}
                    key={key}
                  />
                );
              })
    ...
    <Fragment>
      <tr>
        <td className="p-2 text-left">
          {user.cName.length > 20
            ? user.cName.slice(0, 20) + "..."
            : user.cName}
        </td>
        <td className="p-2 text-left">
          {user.cDescription.length > 30
            ? user.cDescription.slice(0, 30) + "..."
            : user.cDescription}
        </td>
